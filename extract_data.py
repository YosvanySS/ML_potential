...
def extract_data(name_file, structure_type, data_file):
    ...
    with open(data_file, 'a') as db:
        ...
        db.write("Lattice=\""+"{:.3f}".format(lattice_matrix[0][0])+" "+"{:.3f}".format(lattice_matrix[0][1])+" "+"{:.3f}".format(lattice_matrix[0][2])+" "+"{:.3f}".format(lattice_matrix[1][0])+" "+"{:.3f}".format(lattice_matrix[1][1])+" "+"{:.3f}".format(lattice_matrix[1][2])+" "+"{:.3f}".format(lattice_matrix[2][0])+" "+"{:.3f}".format(lattice_matrix[2][1])+" "+"{:.3f}".format(lattice_matrix[2][2])+"\" Properties=species:S:1:pos:R:3:force:R:3 config_type="+structure_type+" energy="+str(energy)+" virial=\""+"{:.8f}".format(stress[0][0])+" "+"{:.8f}".format(stress[0][1])+" "+"{:.8f}".format(stress[0][2])+" "+"{:.8f}".format(stress[1][0])+" "+"{:.8f}".format(stress[1][1])+" "+"{:.8f}".format(stress[1][2])+" "+"{:.8f}".format(stress[2][0])+" "+"{:.8f}".format(stress[2][1])+" "+"{:.8f}".format(stress[2][2])+"\" pbc=\"T T T\"\n")
...
def create_output_list(directory):


    # Pattern to extract the type of calculation and the id from the name of the output files
    pattern = r"(.+?)_(\d+)\.out"

    # List to store the type of calculations sort by ID 
    type_calc_id = {}

    # Iteration for all the files in the directory
    for file in os.listdir(directory):
        if file.endswith(".out"):
            # Using reular expression to extract type of calculation and ID
            match = re.match(pattern, file)
            if match:
                type_calc = match.group(1)
                idx = int(match.group(2))
                # Adding type of calculation to the list
                if idx not in type_calc_id:
                    type_calc_id[idx] = []
                    type_calc_id[idx].append(type_calc)
...
#Name of the file with the index
info_file = 'index.dat'
...
#Total number of structures in the data set
num_id = 1033
...
type_calc = create_output_list(directory)

#Iteration for all the outputs files to extract the data
for  i in range(num_id):
    if i in type_calc:
        structure_type=type_calc[i][0]
        name_file=directory+"/"+structure_type+"_"+str(i)+".out"
        data = extract_data(name_file, structure_type, data_file)

index = 0

#Creating an index file which contains in which line begins the data of each calculation
with open(info_file, 'a') as info:
    for  i in range(num_id):
        if i in type_calc:
            structure=type_calc[i][0]
            index +=1
            command = "grep -no 'Lattice' train.xyz | awk 'NR=="+str(index)+" {print $0}'"
            return_command = subprocess.check_output(command, shell=True, text=True)
            num_line = int(return_command.split(':')[0])-1
            info.write("{:<10} {} \n".format(num_line,structure+"_"+str(i)))
