    ...
    def create_run(self):
        ...
        for i in range(self.cont_str):
            ...
            with open(output_i, 'r') as file:
                ...
                sbp.run('cp ../'+moving_input+' ./', shell=True)
        ...
        # sbp.run('mv -r ntasks_* '+moving_run, shell=True)
        # os.chdir('../')
...
class BashCreator:
    '-----------------------------------------------------------------------'
    # Initializing the instances
    def __init__(self, db_file, bash_file, path_qe, path_parallel, path_timeout, ncpus):
...
if __name__ == "__main__":
    ...
    bash = BashCreator(db_file, bash_file, path_qe, path_parallel, path_timeout, ncpus)
    bash.temp_bash()
