...
class RunCreator:
    ...
    def run_writer_general(self, npools, ntasks, nodes):
        ...
        with open(run_name, 'w') as file_run:
            ...
            file_run.write("#SBATCH --account=mzn@cpu\n")
            file_run.write("#SBATCH --job-name=ntasks_"+str(ntasks)+"\n")
            file_run.write("#SBATCH --output=ntasks_"+str(ntasks)+".out           # nom du fichier de sortie\n")
            file_run.write("#SBATCH --error=ntasks_"+str(ntasks)+"_%j.err          # nom du fichier d'erreur (ici en commun avec la sortie)\n")
            file_run.write("\n")
            
            file_run.write("#SBATCH --ntasks="+str(ntasks)+"\n")
            file_run.write("#SBATCH --ntasks-per-node=40\n")
            file_run.write("#SBATCH --hint=nomultithread\n")
            file_run.write("#SBATCH --time=20:00:00\n")
            file_run.write("#SBATCH --qos=qos_cpu-t3\n")
            file_run.write("\n")
            ...
            file_run.write("mkdir already_done\n")
            file_run.write("\n")
            ...
            file_run.write("\t cp \"${name_list[j]}\".in $WORK/ML/\n")
            file_run.write("\t cd $WORK/ML/\n")
            file_run.write("\n")
            
            file_run.write("\t srun -n "+str(ntasks)+" pw.x -npools "+str(npools)+" -input \"${name_list[j]}\".in > \"${name_list[j]}\".out\n")
            file_run.write("\n")
            
            file_run.write("\t cp \"${name_list[j]}\".out $DIR\n")
            file_run.write("\n")
    ...
    def create_run(self):
        ...
        for i in range(self.cont_str):
            ...
            with open(output_i, 'r') as file:
                ...
                self.run_writer_idris(name_job, npools, ntasks, nodes)
...
class BashCreator:
    ...
    def temp_bash(self):
        ...
        with open(bash_name, 'w') as file_bash:
            ...
            file_bash.write("nconf=$(grep -o 'Lattice=' ../train.xyz | wc -l)\n")
            file_bash.write("\n")
...
if __name__ == "__main__":
    ...
    pseudo_cluster = '/linkhome/rech/genpii01/uex56lt/Pseudo'
