...
class QEWriter:
    ...
    def ang2crystal(self):
        pos_matr_crys = np.zeros((3, self.nat))   # Matrix with atomic positions in crystal units
    ...
    def kpoints(self):
        k_points = np.zeros(3)      # k-points
        
        for i in range(3):
            norm_vect = np.sqrt(np.sum(np.power(self.vect_matr[:, i], 2)))   # sqrt(a_x^2 + a_y^2 + a_z^2), same for b and c vectors
            norm_vect_kspace = 2*mt.pi / norm_vect                      # The norm of the vectors in the reciprocal lattice space 
            k_points[i] = mt.ceil(norm_vect_kspace / self.dk)           # This give us the number of k-points considered depending of the cosidered delta_k. The value is rounded to the next integer.
    ...
    def number_bnd(self):
        _nelec_ = 0     # Variable to count the number of electrons
        ...
        nbnd = mt.ceil(0.6 * _nelec_)      # number of elctronic bands is equal to the number of electrons/2 + a 20% of nbnd for metals regarding pw.x webpage
    ...
    def number_typ(self):
        unique_symbols = set(self.atm_names)        # Set the similar atoms names in unique_symbols tuple
...
class FileReader:
    ...
    def read_file(self):
        ...
        # Tuples to save the cell primitive vectors from the data base (db)
        a, b, c = [], [], []
...
'==========================================================================='
class RunCreator:
    def __init__(self, ont_str, time_exe, folder1_path, folder2_path):
    ...
    def execution(self):
        ...
        for i in range(self.cont_str):
            ...
            command = "/usr/bin/pw.x <"+input_i+" >"+output_i+""
            ...
            if process.poll() is None:
                ...
                print(f" Memory used at this iteration: {memory_usage()} MB")
                print('----------------------------------------------------')
        os.chdir('../')
    ...
    def create_run(self):
        ...
        for i in range(self.cont_str):
            ...
            file.close()
        path_run = '../'+folder1_path
        sbp.run('mv run* '+path_run+'', shell=True)
        os.chdir('../')
...
if __name__ == "__main__":
    ...
    print('----------------------------------')
    print(f" Memory used at the beginning: {memory_usage()} MB")
    print('----------------------------------------------------')
    
    # Ti,e execution of QE in cluster. Format hh:mm:ss
    time_exe = '05:00:00'
    
    
    runner = RunCreator(cont_str, time_exe, folder1_path, folder2_path)
    #runner.execution()
    #os.chdir(folder2_path)       # Changing the repository to execute pw.x code in the local repository      
    command = "bash qe1.sh"
